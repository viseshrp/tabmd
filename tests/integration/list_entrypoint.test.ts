...
function makeNote(id: string, content: string, title: string | null, modifiedAt: number): NoteRecord {
  return {
    id,
    content,
    title,
...
describe('list entrypoint', () => {
  ...
  it('renders notes, filters search, opens notes, renames, and deletes', async () => {
    ...
    vi.stubGlobal('prompt', vi.fn(() => 'Renamed note'));
    ...
    document.getElementById('btn-new-note')?.dispatchEvent(new MouseEvent('click'));
    ...
    expect(document.querySelector('.note-title')?.textContent).toContain('Contains keyword here');

    document.querySelector<HTMLButtonElement>('.action-btn')?.click();
    await new Promise((resolve) => setTimeout(resolve, 0));
    const storedAfterRename = mock.__storageData[STORAGE_KEYS.notes] as Record<string, NoteRecord>;
    ...
    const storedAfterDelete = mock.__storageData[STORAGE_KEYS.notes] as Record<string, NoteRecord>;
    expect(storedAfterDelete.alpha).toBeUndefined();
    expect(document.getElementById('snackbar')?.textContent).toContain('Note deleted');
  });
