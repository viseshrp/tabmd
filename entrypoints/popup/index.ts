import { listNotesSorted } from '../shared/notes';
...
async function bootstrap() {
    ...
    const listContainer = document.getElementById('note-list') as HTMLUListElement;
    const emptyState = document.getElementById('empty-state') as HTMLDivElement;

    try {
        const notes = await listNotesSorted();
        ...
        if (notes.length === 0) {
            ...
            recentNotes.forEach(note => {
                const title = resolveNoteTitle({ title: note.title, content: note.content });
